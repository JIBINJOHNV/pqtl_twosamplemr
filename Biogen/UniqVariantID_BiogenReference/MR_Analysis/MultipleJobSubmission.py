...
for i in range(0,50):
    ...
    os.system(f"cp CisExposure_mr_analysis_sbatch_command.sh Biogen_CisExposureUniqVariants_twosampleMR_Running.R {Dir}")
    os.chdir(Dir)
...
for i in range(0,50):
    Dir=f"{basedir}Part{i}/"
    os.system(f"cp Biogen_TransExposureUniqVariants_twosampleMR_Running.R TransExposure_mr_analysis_sbatch_command.sh {Dir}")
    os.chdir(Dir)
...
for i in range(0,50):
    Dir=f"{basedir}Part{i}/"
    os.system(f"cp Biogen_TransExposureUniqVariants_NoMHC_twosampleMR_Running.R TransExposure_NoMHC_mr_analysis_sbatch_command.sh {Dir}")
    os.chdir(Dir)
...
for i in range(0,50):
    Dir=f"{basedir}Part{i}/"
    os.system(f"cp TransExposure_NoMHC_Unique_mr_analysis_sbatch_command.sh Biogen_TransExposureUniqVariants_NoMHC_Unique_twosampleMR_Running.R {Dir}")
    os.chdir(Dir)
