...
os.system("mkdir -p CombinedResultsfromAllBatches")
...
all_pqtlfiles=["All_significannt_cis_exposure_AfterQC_LDclumping.csv","All_significannt_trans_exposure_AfterQC_LDclumping_MHCRemoval.csv","All_significannt_CisTransexposure_BeforeQC_LDclumping.csv","All_significannt_trans_exposure_AfterQC_LDclumping_NoMHC_Unique.csv","All_significannt_trans_exposure_AfterQC_LDclumping.csv"]

for file in all_pqtlfiles :
    Files=glob.glob(f"Part*/{file}")
    master_df=pd.DataFrame()
    for part_file in Files:
        part_file_df=pd.read_csv(part_file)
...
for gwasname in gwasnames:
    for cistran in cis_trans:
        ...
        #CisTransexposure_df = pd.read_csv(f"CombinedResultsfromAllBatches/{gwasname}/All_significannt_CisTransexposure_BeforeQC_LDclumping.csv")
        with pd.ExcelWriter(f"CombinedResultsfromAllBatches/{gwasname}/{file_prefix}_CompleteMR_AnalysisResults.xlsx", engine='xlsxwriter') as writer:
            ...
            #CisTransexposure_df.to_excel(writer, sheet_name='All_CisTrans_Pqtls')
