...
files_df <- fread("/edgehpc/dept/human_genetics/users/jjohn1/Cis_Trans/1kg_clump/Decode/Decode_Proteins_TrnnscriptSTart_End_Hg38_Part1.csv")
tss<-files_df
...
for ( file in unique(tss$Filename) ){
    ...
    vcf_file<-glue("/edgehpc/dept/human_genetics/users/jjohn1/Pqtl_Summarystatistics/DecodeIcelanders_35559_4907Protein/VcfFiles/{substr(file, 1, nchar(file) - 7)}.vcf.gz")
