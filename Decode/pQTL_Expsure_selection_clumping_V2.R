...
files_df <- fread("/edgehpc/dept/human_genetics/users/jjohn1/Cis_Trans/1kg_clump/Decode/Reanalysis/Decode_Proteins_TrnnscriptSTart_End_Hg38.csv")
tss<-files_df
...
for ( file in unique(tss$Filename) ){
    ...
    vcf_file<-glue("/edgehpc/dept/human_genetics/users/jjohn1/Pqtl_Summarystatistics/DecodeIcelanders_35559_4907Protein/VcfFiles/VCF_Files/{substr(file, 1, nchar(file) - 7)}.vcf.gz")
...
#Rscript pQTL_Expsure_selection_clumping.R
