...
##Go to ForMeta/MetaP_AnalysisResults  folder
...
def save_dataframe_to_excel_with_highlighting(dataframe, output_filename, gwas_names, pqtltypes,pqt_gwas_dict):
    ...
    gene_symbol_col_idx = dataframe.columns.get_loc('UniProt_ID') + 1
...
def pqtltype_merge(cis_files_dict,pqtltype,pvalue_cutof1,pvalue_cutof2,pqt_gwas_dict):
    cis_df=pd.DataFrame(columns=["UniProt_ID"])
    
    for cis_file in cis_files_dict.keys():
        df=pd.read_csv(cis_file)
        df=df[["UniProt_ID"]].drop_duplicates()
        cis_df=pd.merge(cis_df,df,on="UniProt_ID",how="outer").drop_duplicates()
    
    for cis_file in cis_files_dict.keys():
        df=pd.read_csv(cis_file)
        df2=df[["UniProt_ID"]+[x for x in df.columns if   "IVWDelta_Pvalue" in x or "metap_MRIVWtest"  in x  or "IVWDelta_SNPs"  in x or "IVWDelta_Beta" in x or "IVWDelta_nSNPs"  in x ]]
        rename_columns={x:f'{pqtltype}_{x}' for x in df.columns if x.endswith("_Beta") or x.endswith("_Pvalue") or x.endswith("_SNPs") or x.endswith("_nSNPs") }
        df2=df2.rename(columns=rename_columns)
        df2=df2.rename(columns={"metap_MRIVWtest":f'{pqtltype}_{cis_files_dict[cis_file]}_metap_Pvalue'})
        ...
        df2=df2.sort_values(Lpvaluename).drop_duplicates(subset='UniProt_ID', keep='first')
        df2[pqtltype+"_"+cis_files_dict[cis_file]+"_count"]=df2[pvalue_columns].apply(lambda row: (row < pvalue_cutof2).sum(), axis=1)
        cis_df=pd.merge(cis_df,df2,on="UniProt_ID",how="outer").drop_duplicates() #.drop("LwestPvalue",axis=1)
...
pqt_gwas_dict={'CIS_ASD_PGC_LwestPvalue':'CIS_ASD_PGC_count', 
               'CIS_PGC3_SCZ_NoUKB_LwestPvalue':'CIS_PGC3_SCZ_NoUKB_count', 
               'CIS_Depression_iPSYCH_2023_LwestPvalue':'CIS_Depression_iPSYCH_2023_count',
               'CIS_BIP_PGC3_noukb_LwestPvalue':'CIS_BIP_PGC3_noukb_count', 
               'CIS_PGC_AN2_LwestPvalue':'CIS_PGC_AN2_count', 
               'CIS_ADHD2022_iPSYCH_deCODE_LwestPvalue':'CIS_ADHD2022_iPSYCH_deCODE_count',
               'CIS_Cognition_LwestPvalue':'CIS_Cognition_count',
               'TransExposureNoMHC_ASD_PGC_LwestPvalue':'TransExposureNoMHC_ASD_PGC_count', 
               'TransExposureNoMHC_PGC3_SCZ_NoUKB_LwestPvalue':'TransExposureNoMHC_PGC3_SCZ_NoUKB_count', 
               'TransExposureNoMHC_Depression_iPSYCH_2023_LwestPvalue':'TransExposureNoMHC_Depression_iPSYCH_2023_count',
               'TransExposureNoMHC_BIP_PGC3_noukb_LwestPvalue': 'TransExposureNoMHC_BIP_PGC3_noukb_count', 
               'TransExposureNoMHC_PGC_AN2_LwestPvalue': 'TransExposureNoMHC_PGC_AN2_count', 
               'TransExposureNoMHC_ADHD2022_iPSYCH_deCODE_LwestPvalue': 'TransExposureNoMHC_ADHD2022_iPSYCH_deCODE_count',
               'TransExposureNoMHC_Cognition_LwestPvalue': 'TransExposureNoMHC_Cognition_count'}


pvalue_cutof1=0.05
pvalue_cutof2=0.01

all_pqtl_df=pd.DataFrame(columns=["UniProt_ID"])
...
for pqtltype in pqtltypes.keys():
    cis_files_dict={ 
    f'Biogen_Decode_pQTL_PGC3_SCZ_NoUKB_{pqtltype}_British_IVDelt_MetapAnalysis.csv':"PGC3_SCZ_NoUKB",
    ...
    f'Biogen_Decode_pQTL_Cognition_{pqtltype}_British_IVDelt_MetapAnalysis.csv':"Cognition"
    }
    ...
    all_pqtl_df=pd.merge(all_pqtl_df,result_df,on="UniProt_ID",how="outer").drop_duplicates()

 
    #f'Biogen_Decode_pQTL_ASD_PGC_{pqtltype}_British_IVDelt_MetapAnalysis.csv':'ASD_PGC',
...
all_pqtl_df_cis_nomhc=["UniProt_ID"]+[x for x in all_pqtl_df.columns if x.startswith("CIS_") or x.startswith("TransExposureNoMHC_") ]
...
def order_function(unorderedcolumns):
    ...
    cogdeta=[x for x in unorderedcolumns if "Cognition" in x]
    beta_order=sczbeta+bipbeta+mddbeta+adhdeta+asdbeta+eatingbeta+cogdeta
    return(beta_order)
...
geenecols=order_function([x for x in all_pqtl_df_cis_nomhc_df.columns if x.endswith("_Gene_Symbol")])
ordered_cols=["UniProt_ID"]+geenecols+totalcountcols+countcols+lpvaluecols+pvaluecols+betacols+nsnpscols+snpscols
...
all_pqtl_df_withmhc_unique=["UniProt_ID"]+[x for x in all_pqtl_df.columns if x not in all_pqtl_df_cis_nomhc ]
