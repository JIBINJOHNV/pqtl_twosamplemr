## For metap analysis using uniprot Id
...
gwasnames <- c("ASD_PGC","BIP_PGC3_noukb","Depression_iPSYCH_2023","PGC_ADHD2022_iPSYCH_deCODE","PGC_AN2"  )
...
pqtltype1 <- "Decode"
pqtltype2 <- "Biogen"
exclude_columns <- c("UniProt_ID", "outcome")
...
for (gwasname in gwasnames) {
  for (prefix in prefixes) {
    ...
    decode <- setNames(decode, gsub("^exposure$", "Decode.PQTL_exposure", colnames(decode)))
    decode <- setNames(decode, gsub("^Gene_Symbol$", "Decode.PQTLGene_Symbol", colnames(decode)))
    ...
    biogen <- setNames(biogen, gsub("^Gene_Symbol$", "Biogen.PQTL_Gene_Symbol", colnames(biogen)))

    ## Merge biogen and decode data frames
    decode_biogen <- merge(decode, biogen, by = c("UniProt_ID", "outcome"), all = TRUE)
    ...
    new_order <- c(c("UniProt_ID","outcome"), first_cols, c(glue("{gwasname}_metap_MRIVWtest")), second_cols,third_cols,fourth_cols,fifth_cols,sixth_cols,seventh_cols,
                    setdiff(colnames(decode_biogen), c("UniProt_ID","outcome", first_cols,glue("{gwasname}_metap_MRIVWtest"), second_cols,third_cols,fourth_cols,fifth_cols,sixth_cols,seventh_cols)))

    decode_biogen_reordered_df <- decode_biogen[, new_order]

    if (grepl("CisExposure_" ,prefix )) {decode_biogen_reordered_df <- decode_biogen_reordered_df %>%rename_with(~ paste("CIS", ., sep = "_"), -c("UniProt_ID", "outcome")) }
    if (grepl("TransExposure_British_",prefix )) {decode_biogen_reordered_df <- decode_biogen_reordered_df %>%rename_with(~ paste("TransExposure", ., sep = "_"), -c("UniProt_ID", "outcome")) }
    if (grepl("TransExposureNoMHC_" ,prefix )) {decode_biogen_reordered_df <- decode_biogen_reordered_df %>%rename_with(~ paste("TransExposureNoMHC", ., sep = "_"), -c("UniProt_ID", "outcome")) }
    if (grepl("TransExposureNoMHCUnique_" ,prefix )) {decode_biogen_reordered_df <- decode_biogen_reordered_df %>%rename_with(~ paste("TransExposureNoMHCUnique", ., sep = "_"), -c("UniProt_ID", "outcome")) }
