...
file_list <- list.files("/edgehpc/dept/human_genetics/users/jjohn1/Analysis/", pattern = "*vcf.gz", full.names = TRUE)
file_list <- file_list[!grepl("\\.tbi$", file_list)]
file_list <- sort(file_list)


tss<-fread("/edgehpc/dept/human_genetics/users/jjohn1/Cis_Trans/All_Exposure_TSS_chr_index_3k.csv")
tss<-tss[,c("Symbol","Chromosome_edit","Biomarker_TSS_Start","Biomarker_TSS_End")]
...
ld_pruning <- function(df2) {
    ...
    tryCatch({df3 <- suppressMessages(suppressWarnings({
                            ...
                            bfile = "/edgehpc/dept/human_genetics/users/jjohn1/Referencefile_h19/ukb_imp_chr1_22_mac50_info07_b0_7_patched_bfiles_SNPonly_vcfCooker_rsid_nodup")  }))},
...
for (file in file_list[1:length(file_list)]) {
    ...
    tryCatch({ vcf <- readVcf(file) }, error = function(e) {})
...
if (nrow(df)>0) {
    ...
    collated_protein_dis <- merge(df, tss, by.x = c("id"),by.y=c("Symbol"))
    ...
    all_significantPQTL_variant<-rbind(all_significantPQTL_variant,collated_protein_dis)
    ...
    trans<-trans[,c("seqnames","start","ID","REF","ALT","ES","SE","P","AF","SI","SS","id")]
    cis<-cis[,c("seqnames","start","ID","REF","ALT","ES","SE","P","AF","SI","SS","id")]

        }
    if (nrow(cis)>0){

        selected_exposure_df<-ld_pruning(cis)
        all_selected_cis_exposure_df<-rbind(all_selected_cis_exposure_df,selected_exposure_df)
    ...
    if (nrow(trans)>0){
        ...
        all_selected_trans_exposure_df<-rbind(all_selected_trans_exposure_df,selected_exposure_df)
        ...
        all_selected_trans_exposurenomhc_df<-rbind(all_selected_trans_exposurenomhc_df,selected_exposure_omhc_df)
...
write.csv(all_selected_cis_exposure_df,glue("All_significannt_cis_exposure_AfterQC_LDclumping_Part{PART1}.csv"))
write.csv(all_selected_trans_exposure_df,glue("All_significannt_trans_exposure_AfterQC_LDclumping_Part{PART1}.csv"))
write.csv(all_selected_trans_exposurenomhc_df,glue("All_significannt_trans_exposure_AfterQC_LDclumping_MHCRemoval_Part{PART1}.csv"))
write.csv(all_significantPQTL_variant,glue("All_significannt_CisTransexposure_BeforeQC_LDclumping_Part{PART1}.csv"))

no_cisvariant_df <- data.frame(WithoutCisVariant = no_cisvariant)
write.csv(no_cisvariant_df,glue("GenesWithoutCisvariant_Part{PART1}.csv"))
no_transvariant_df <- data.frame(WithoutTransVariant = no_transvariant)
write.csv(no_transvariant_df,glue("GenesWithoutTransvariant_Part{PART1}.csv"))
