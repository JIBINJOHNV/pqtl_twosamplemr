...
for pqtltype in ["CisExposure","TransExposureNoMHC"]:
    ...
    for Disease in ["Depression_iPSYCH_2023","BIP_PGC3_noukb","PGC3_SCZ_NoUKB","Cognition"]:
        ...
        for exposure in Exposures:
            ...
            if selected_df.shape[0]>29:
                try:
                    biogen=selected_df[ (selected_df["SNP"].str.contains("UKB-PPP|Biogen")) & (selected_df["UKBB_PPP_exposure"].notna()) ]
                    biogen["SNP"]=biogen["SNP"].replace("Biogen","UKB-PPP")
                ...
                try:
                    decode=selected_df[ (selected_df["SNP"].str.contains("Decode")) & (selected_df["DECODE_exposure"].notna()) ]
                    decode=decode[['SNP', 'nSNP','P', 'b', 'se','group', 'up', 'lo', 'MAF']].drop_duplicates()
