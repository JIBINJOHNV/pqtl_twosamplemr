...
conditions = [
    ...
    ((df2["FDR_P_value"] > 0.05) & (df2['Lowest_Pvalue'] < 0.01) & df2["FDR_P_value"].notna()  & df2["BONF_P_Value"].notna())
]

values=["strictly significant","FDR","nominally significant"]
df2["shapee_vale"]=np.select(conditions, values, default='Unknown')

df3=df2.pivot(index=['Gene symbol','CIS/TRANS','Estimate_Direction'], columns='Outcome', values=['Lowest_Pvalue',"shapee_vale","BONF_P_Value","FDR_P_value"]).reset_index()
...
bnf_05_m1=pd.melt(bnf_df,id_vars=['Gene symbol_', 'CIS/TRANS_','Estimate_Direction_'],
...
bnf_05_m2=pd.melt(bnf_df,id_vars=['Gene symbol_', 'CIS/TRANS_','Estimate_Direction_'],
...
bnf_05_m=pd.merge(bnf_05_m1,bnf_05_m2,on=['Gene symbol_', 'CIS/TRANS_','Phenotype','Estimate_Direction_'])
...
bnf_05_m=bnf_05_m[bnf_05_m["P-value"]>=2]
